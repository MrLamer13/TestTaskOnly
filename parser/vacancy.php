...
if (($handle = fopen("vacancy.csv", "r")) !== false) {
    while (($data = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        if ($PROP['ACTIVITY'] == 'Проектная/Временная работа') {
            $PROP['ACTIVITY'] = $arProps['ACTIVITY']['Проектная'];
        }

        if ($PROP['TYPE'] != $arProps['TYPE']['Рабочие']) {
            $PROP['TYPE'] = $arProps['TYPE']['Продажи'];
        }

        switch ($PROP['FIELD']) {
            case 'Начальный уровень, Мало опыта':
                $PROP['FIELD'] = $arProps['FIELD']['Персонал'];
                break;
            case 'Закупки и логистика':
                $PROP['FIELD'] = $arProps['FIELD']['Закупки'];
                break;
            case 'Технология и качество':
                $PROP['FIELD'] = $arProps['FIELD']['Техническое развитие'];
                break;
            case 'Развитие бизнеса':
                $PROP['FIELD'] = $arProps['FIELD']['Служба развития производственной системы'];
                break;
            case 'Лесозаготовка':
                $PROP['FIELD'] = $arProps['FIELD']['Производство'];
                break;
            case 'Строительство, Недвижимость':
                $PROP['FIELD'] = $arProps['FIELD']['Технический департамент'];
                break;
        }
