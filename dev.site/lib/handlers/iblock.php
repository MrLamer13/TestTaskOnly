...
class Iblock
{
    public static function addLog(array $arFields): void
    {
        ...
        $elForLog = \CIBlockElement::GetByID($arFields["ID"])->GetNext();

        $logIBlockSectionName = $elForLog["IBLOCK_NAME"] . " - " . $elForLog["IBLOCK_CODE"];
        $logIBlockSection = \CIBlockSection::GetList(
            [],
            ["NAME" => $logIBlockSectionName]
        )->GetNext();
        if ($logIBlockSection === false) {
            $section = new \CIBlockSection;
            $logIBlockSectionId = $section->Add([
                "ACTIVE" => "Y",
                "IBLOCK_ID" => $logIBlockID,
                "NAME" => $logIBlockSectionName,
            ]);
        } else {
            $logIBlockSectionId = $logIBlockSection["ID"];
        }

        $chainName = [];
        $chainName[] = \CIBlock::GetByID($arFields["IBLOCK_ID"])->GetNext()["NAME"];
        $chainSection = \CIBlockSection::GetNavChain($arFields["IBLOCK_ID"], $elForLog["IBLOCK_SECTION_ID"]);
        self::sectionNameInArray($chainSection, $chainName);
        $chainName[] = $elForLog["NAME"];
        $chainNameString = implode(" -> ", $chainName);
        ...
        $el->Add([
            ...
            "NAME" => $arFields["ID"],
            "ACTIVE" => "Y",
            "ACTIVE_FROM" => $elForLog["TIMESTAMP_X"],
            "PREVIEW_TEXT" => $chainNameString,
        ]);
    }

    private static function sectionNameInArray(\CIBlockResult $chainSection, array &$arr): void
    {
        $chainLink = $chainSection->GetNext();
        if ($chainLink === false) {
            return;
        }
        $arr[] = $chainLink["NAME"];
        self::sectionNameInArray($chainSection, $arr);
    }
